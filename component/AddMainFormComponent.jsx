...
import { meansTypeList,meansTypeAdd,meansTypeUpdate,meansTypeDelete,LanguageFind } from './apiService';
const AddMainFormComponent = () => {
    ...
    const openModal = () => {
        setTitle(LanguageFind('type_add'));
    ...
    const handleEdit = (item) => {
        setTitle(LanguageFind('type_update'));
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 5, marginHorizontal: 10, backgroundColor: 'white', borderColor: '#e9e7e7', borderWidth: 1, padding: 5, borderRadius: 5 }}>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        <TouchableOpacity onPress={() => handlePlay(item)}>
                            <FontAwesome  name="play" size={20} color="orange" style={{ marginLeft: 10, marginTop: -2 }} />
                        </TouchableOpacity>
            ...
            <View style={styles.listItem}>
                <View style={styles.formContainer}>
                    <View style={[styles.tabMenu, { backgroundBottomColor: 'white', borderBottomColor: '#e9e7e7', borderBottomWidth: 1, paddingBottom: 5 }]}>
                        <Text style={[styles.buttonText]}>{LanguageFind('card_meaning_types')}</Text>
                    </View>
                    ...
                    <View style={[styles.tabMenu, { backgroundColor: '#e9e7e7', borderColor: '#e9e7e7', borderWidth: 1, marginBottom: 10, padding: 5, borderRadius: 5 }]}>
                            ...
                            <Text style={styles.buttonText}>{LanguageFind('type_create')}</Text>
                ...
                <Modal isVisible={isModalVisible} onBackdropPress={closeModal}>
                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <View style={[styles.tabMenu, { backgroundBottomColor: 'white', borderBottomColor: '#e9e7e7', borderBottomWidth: 1, paddingBottom: 5 }]}>
                            ...
                            <Text style={{marginBottom:'2px'}}><b>{LanguageFind('type_name')}</b></Text>
                            <TextInput
                                

                                style={[styles.input, { backgroundColor: 'white' }]}
                                value={formData.mt_name}
                                placeholder={LanguageFind('enter_the_type')}
                                onChangeText={(text) => handleInputChange('mt_name',text)}
                                name='mt_name'

                            />

                            <Text style={{marginBottom:'2px'}}><b>{LanguageFind('type_short_code')}</b></Text>
                            <TextInput
                                style={[styles.input, { backgroundColor: 'white' }]}
                                value={formData.mt_short_name}
                                placeholder={LanguageFind('enter_the_short_type')}
                                onChangeText={(text) => handleInputChange('mt_short_name',text)}
                                name='mt_short_name'
                            />
                            
                            <View style={styles.tabMenu}>
                                <Button title={LanguageFind('save')} style={{width:'100%'}} onPress={handleSubmit} />
                                <Button title={LanguageFind('close')} style={{width:'100%'}} onPress={closeModal} />
                            </View>
